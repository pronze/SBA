...
public class SBACommand {
    ...
    private void commandReload(
        ...
        SBAUtil.reloadPlugin(SBAHypixelify.getPluginInstance());
    ...
    private void commandGenerate(
            final @NotNull CommandSender sender,
            final @NotNull @Argument(value = "gamemode", parserName = "gamemode") String gameMode,
            final @NotNull @Argument(value = "maps", parserName = "maps") List<String> maps
    ) {
        Logger.trace("Generating Games Inventory file for game mode: {}", gameMode);
        final var file = new File(SBAHypixelify.getPluginInstance().getDataFolder(), "games-inventory/" + gameMode + ".yml");
        if (file.exists()) {
