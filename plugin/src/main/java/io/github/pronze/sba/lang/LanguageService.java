...
@Getter
public class LanguageService extends LangService {
    ...
    public void onEnable() {
        ...
        internalLanguageDefinition = new Gson().fromJson(new InputStreamReader(Objects.requireNonNull(LanguageService.class.getResourceAsStream("/language_definition.json"))), LanguageDefinition.class);
        if (internalLanguageDefinition == null) {
            logger.error("Can't load default Language Definition for SBA!");
            return;
        }
                    ...
                    try {
                        return LayeredTranslationContainer.of(
                                GsonConfigurationLoader
                                        .builder()
                                        .source(() -> new BufferedReader(new InputStreamReader(Objects.requireNonNull(LanguageService.class.getResourceAsStream("/" + entry.getValue())))))
                                        .build()
                                        .load()
                        );
