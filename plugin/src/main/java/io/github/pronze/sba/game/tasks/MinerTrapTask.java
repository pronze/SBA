                ...
                .forEach(team -> arena.getGame().getConnectedPlayers()
                        ...
                        .forEach(player -> {
                            ...
                            if (arena.getStorage().getTargetBlockLocation(team).orElseThrow().distanceSquared(player.getLocation()) <= radius) {
                                ...
                                SBALanguageService
                                        .getInstance()
                                        .get(LangKeys.TEAM_MINER_TRAP_TRIGGERED_MESSAGE).replace("%team%", arena.getGame().getTeamOfPlayer(player).getName())
                                        .send(PlayerMapper.wrapPlayer(player).as(SBAPlayerWrapper.class));

                                var title = SBALanguageService
                                        .getInstance()
                                        .get(LangKeys.TEAM_MINER_TRAP_TRIGGERED_TITLE)
                                        .toString();

                                var subTitle = SBALanguageService
                                        .getInstance()
                                        .get(LangKeys.TEAM_MINER_TRAP_TRIGGERED_SUBTITLE)
                                        .toString();
