...
public class TrapTask extends AbstractGameTaskImpl {
    ...
    public void run() {
        ...
        if (!storage.areBlindTrapEnabled()) {
            return;
        }
        ...
        for (var runningTeam : gameWrapper.getRunningTeams()) {
            ...
            for (var gamePlayer : gameWrapper.getConnectedPlayers()) {
                ...
                if (runningTeam.getTargetBlockLocation().isInRange(gamePlayer.getLocation(), radius)) {
                    ...
                    gameWrapper.getStorage().setPurchasedBlindTrap(runningTeam, false);
                    gamePlayer.addPotionEffect(PotionEffectHolder.of(new PotionEffect(PotionEffectType.BLINDNESS, 20 * 3, 2)));
