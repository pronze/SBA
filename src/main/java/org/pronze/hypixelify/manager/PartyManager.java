...
public class PartyManager implements org.pronze.hypixelify.api.party.PartyManager{
    ...
    public void disband(Player leader) {
        ...
        if(party.getPlayers() != null && !party.getPlayers().isEmpty()) {
            for (Player pl : party.getAllPlayers()) {
                if (pl != null) {
                    if (pl.isOnline()) {
                        for (String str : SBAHypixelify.getConfigurator().config.getStringList("party.message.disband")) {
                            pl.sendMessage(ShopUtil.translateColors(str));
                        }
                    }
                    final PlayerDatabase plDatabase = SBAHypixelify.getDatabaseManager().getDatabase(pl);
                    if (plDatabase != null) {
        ...
        SBAHypixelify.getDatabaseManager().getDatabase(leader).setIsInParty(false);
    ...
    public boolean isInParty(Player player) {
        if (SBAHypixelify.getDatabaseManager().getDatabase(player) != null)
            return SBAHypixelify.getDatabaseManager().getDatabase(player).isInParty();
    ...
    public void addToParty(Player player, org.pronze.hypixelify.api.party.Party party) {
        ...
        for (Player p : parties.get(leader).getAllPlayers()) {
            ...
            for (String message : SBAHypixelify.getConfigurator().config.getStringList("party.message.accepted")) {
        ...
        if(parties.get(db.getPartyLeader()).getPlayers() != null && !parties.get(db.getPartyLeader()).getPlayers().isEmpty()) {
            for (Player pl : parties.get(db.getPartyLeader()).getAllPlayers()) {
                if (pl != null && pl.isOnline()) {
                    for (String st : SBAHypixelify.getConfigurator().config.getStringList("party.message.offline-quit")) {
    ...
    public void kickFromParty(Player player) {
        ...
        if (player.isOnline()) {
            for (String st : SBAHypixelify.getConfigurator().config.getStringList("party.message.got-kicked")) {
        ...
        if (parties.get(leader).getPlayers() != null) {
            for (Player pl : parties.get(leader).getAllPlayers()) {
                if (pl != null && pl.isOnline()) {
                    for (String st : SBAHypixelify.getConfigurator().config.getStringList("party.message.kicked")) {
                        pl.sendMessage(ShopUtil.translateColors(st).replace("{player}", player.getDisplayName()));
                    }
                }
            }
    ...
    public void removeFromInvitedParty(Player player) {
        PlayerDatabase database = SBAHypixelify.getDatabaseManager().getDatabase(player);
            ...
            if (!partyLeader.getUniqueId().equals(player.getUniqueId())) {
                if (party.getAllPlayers() != null && !party.getAllPlayers().isEmpty()) {
                    for (Player pl : party.getAllPlayers()) {
                        if (pl != null && pl.isOnline() && !player.getUniqueId().equals(pl.getUniqueId())) {
                            for (String st : SBAHypixelify.getConfigurator().config.getStringList("party.message.offline-left")) {
                ...
                for (Player pl : party.getAllPlayers()) {
                    if (pl != null && !pl.equals(partyLeader)) {
                        ...
                        org.pronze.hypixelify.api.database.PlayerDatabase plDatabase = SBAHypixelify.getDatabaseManager().getDatabase(pl);
                        if (plDatabase != null) {
                            plDatabase.setIsInParty(false);
                            plDatabase.setPartyLeader(null);
                        }
